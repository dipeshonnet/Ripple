...
(function () {
  ...
  const H = window.ArenaLeadMgrViewHelpers;
  if (!H) { console.error('Arena lead/manager helper module not loaded'); return; }

  const {
    escapeHtml, priorityChip, dueLabel, sparkline, configuredAccountRewardPotential,
    ragBadge, usd, pct, roleForScope, configuredMetricIds, kpiNameText,
    kpiIdsMatching, outcomeDriverIds, defaultActionKpiId, isConfiguredSlaHealthKpi,
    metricHelp, weekTrendForTeam, latestRows, kpiScore, outcomeScore, outcomeRag,
    outcomeWow, clientOutcomeMetrics, trendCard, impactRange, driverTree, rootCauseChips,
  } = H;
